...
testmoo.db census can produce native code. The active frontend goal is now to
...
native chain and must not be admitted by general dispatch. Owner-backed
sources are implemented for list-tail values with an audited move-to-home
transfer and for property results whose runtime helper normalizes every
successful result to an owned `Var`. They store the home index; capture and
retained native resume validate that the index is in range and the home is
...
sufficient. Property results were admitted only after their helper acquired an
explicit owned-result contract; the earlier generic assignment of homes without
that contract caused database-startup heap corruption. Other complex producer
classes remain disabled until their runtime helpers have an equally explicit
move/reference contract.

Owner-backed reconstruction does not by itself authorize moving a value into a
call boundary. Boundary argument analysis currently moves only
`JIT_OWNERSHIP_OWNED` values. `JIT_OWNERSHIP_OWNED_PROPERTY` values remain
retained by their owner home across the call until a separate proof covers the
callee's consumption, exceptional exits, and caller-resume cleanup. This keeps
the reconstruction contract and the argument-transfer contract independent.
...
After the first VM-call bridge stage, an testmoo.db objects #0 through #25 sample
...
After enabling the VM bridge for every fixed-ID built-in call, the same testmoo.db
...
5. **Reduce deopt metadata and register pressure.** Implement the
   ...
   The next measured pass made local-snapshot pruning instruction- and
   CFG-aware, encoded borrowed-local continuation sources directly, and
   released ownership, escape, use-count, owned-slot, and integer-list analysis
   arrays with compiler IR. Static value types, tag-slot indices, and constant
   SSA IDs now use bounded 16-bit tables; programs exceeding those table bounds
   conservatively retain the uncompressed behavior. Against the committed
   `#463:sha1` baseline of 93,049 metadata bytes and 133,808 machine-code bytes,
   this reduced retained metadata to 74,111 bytes before execution (20.35%) and
   74,271 bytes after warmed execution (20.18%), with 121,376 machine-code
   bytes, zero SHA1 deopts, and a 10.986-second median warmed
   `player:test(300000)` run versus the 11.253-second baseline. Use 121,376
